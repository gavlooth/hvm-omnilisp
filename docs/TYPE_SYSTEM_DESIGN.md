# OmniLisp Type System Design: Gradual Multiple Dispatch

## Executive Summary

OmniLisp uses **Gradual Multiple Dispatch** - a hybrid approach combining:
- **Lisp's runtime flexibility** - Programmer is in control
- **Julia's multiple dispatch semantics** - Dispatch on all argument types
- **Static type safety** - Compile-time resolution when types are known

**What is Gradual Multiple Dispatch?**

Like "gradual typing", gradual dispatch provides a spectrum between static and dynamic:
- **Fully typed code** → compile-time method resolution (static)
- **Untyped code** → runtime type checking and dispatch (dynamic)
- **Partially typed** → static where possible, dynamic fallback
...
Gradual multiple dispatch is achieved through **multiple `define` declarations** with the same name but different type constraints:
...
| Language | Dispatch | When Resolved | Ambiguity | Flexibility |
|----------|----------|---------------|-----------|------------|
| Haskell | Type classes | Compile-time | Compile error | Low (rigid) |
| Java/C# | Single dispatch | Compile-time | Compile error | Low |
| Common Lisp | Dynamic multiple | Runtime | Runtime error | High |
| Julia | Dynamic multiple | Runtime | Runtime heuristic | High |
| **OmniLisp** | **Gradual multiple** | **Both** | **Compile-time detect** | **High** |

**OmniLisp's gradual approach:** Resolve at compile-time when types are known, fall back to runtime dispatch for dynamic code. Best of both worlds.
...
**Gradual Multiple Dispatch** gives OmniLisp the best of all worlds:

1. **Flexibility of Lisp** - Runtime dynamism when needed
2. **Safety of static types** - Compile-time resolution when types are known
3. **Power of multiple dispatch** - Dispatch on all argument types
4. **Gradual spectrum** - Add types incrementally, get proportional static guarantees

The key insight: **Analyze what you can at compile time, fall back to runtime when you can't.**

```lisp
;; Untyped: runtime dispatch
(define process [x] (do-something x))

;; Typed: compile-time dispatch
(define process [x {Int}] (+ x 1))
(define process [x {String}] (string-length x))

;; Gradual: mix as needed
...
- More powerful than Common Lisp (parametric types + gradual dispatch)
...
- Gradual Typing: Siek & Taha, "Gradual Typing for Functional Languages"
